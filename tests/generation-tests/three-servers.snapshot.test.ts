...
const ENDPOINT_FILE = path.resolve(
  OUTPUT_DIR,
  'endpoints',
  'get-multi-server-weather.ts',
);

describe('generateApi snapshot three OpenAPI servers', () => {
  beforeEach(async () => {
    await fs.rm(OUTPUT_DIR, { recursive: true, force: true });
    await fs.mkdir(path.dirname(OUTPUT_DIR), { recursive: true });
  });

  it('generates endpoint matching snapshot (spec defines three servers)', async () => {
    await generateApi(
      defineConfig({
        input: INPUT_FILE,
        output: OUTPUT_DIR,
        ...
        filterEndpoints: [/^getMultiServerWeather$/i],
      }),
    );
