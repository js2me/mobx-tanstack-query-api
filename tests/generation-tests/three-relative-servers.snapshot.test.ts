...
const ENDPOINT_FILE = path.resolve(
  OUTPUT_DIR,
  'endpoints',
  'get-relative-servers-ping.ts',
);

describe('generateApi snapshot three relative OpenAPI servers', () => {
  beforeEach(async () => {
    await fs.rm(OUTPUT_DIR, { recursive: true, force: true });
    await fs.mkdir(path.dirname(OUTPUT_DIR), { recursive: true });
  });

  it('generates endpoint with baseUrl from last relative server url', async () => {
    await generateApi(
      defineConfig({
        input: INPUT_FILE,
        output: OUTPUT_DIR,
        ...
        filterEndpoints: [/^getRelativeServersPing$/i],
      }),
    );
