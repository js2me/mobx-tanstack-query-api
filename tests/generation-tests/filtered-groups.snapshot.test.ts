...
const OUTPUT_DIR = path.resolve(
  __dirname,
  './__generated__/filtered-groups-snapshot',
);
...
describe('generateApi snapshot filtered-groups (big-schema fixture)', () => {
  beforeEach(async () => {
    await fs.rm(OUTPUT_DIR, { recursive: true, force: true });
    await fs.mkdir(path.dirname(OUTPUT_DIR), { recursive: true });
  });

  it('generates grouped endpoints-per-file, data-contracts and meta in one snapshot bundle', async () => {
    await generateApi(
      defineConfig({
        cleanOutput: true,
        noBarrelFiles: true,
