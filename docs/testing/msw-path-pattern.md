...
Returns a **full URL string** for [MSW](/recipes/connect-msw.html): **`baseUrl`** + path, with **`{segment}`** turned into MSW **`:segment`**. No query string. If your real path has extra bits not in **`configuration.path`**, adjust the string yourself.
